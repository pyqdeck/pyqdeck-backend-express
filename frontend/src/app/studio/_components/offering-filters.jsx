'use client';

import * as React from 'react';
import { Filter, X, Check } from 'lucide-react';
import { useRouter, usePathname, useSearchParams } from 'next/navigation';
import { DropdownAction } from '@/components/dropdown-action';
import {
  DropdownMenuItem,
  DropdownMenuSeparator,
  DropdownMenuLabel,
} from '@/components/ui/dropdown-menu';
import { Button } from '@/components/ui/button';

export function OfferingFilters({
  universities = [],
  branches = [],
  semesters = [],
}) {
  const router = useRouter();
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const [uniSearch, setUniSearch] = React.useState('');
  const [branchSearch, setBranchSearch] = React.useState('');
  const [semSearch, setSemSearch] = React.useState('');

  const currentUniId = searchParams.get('universityId') || 'all';
  const currentBranchId = searchParams.get('branchId') || 'all';
  const currentSemesterId = searchParams.get('semesterId') || 'all';

  // Filter universities based on search
  const filteredUniversities = universities.filter(
    (uni) =>
      uni.name.toLowerCase().includes(uniSearch.toLowerCase()) ||
      uni.shortName?.toLowerCase().includes(uniSearch.toLowerCase())
  );

  // Filter branches based on selected university and search
  const filteredBranches = branches.filter((branch) => {
    const matchesUni =
      currentUniId === 'all' ||
      (branch.universityId?.id || branch.universityId) === currentUniId;
    const matchesSearch =
      branch.name.toLowerCase().includes(branchSearch.toLowerCase()) ||
      branch.shortName?.toLowerCase().includes(branchSearch.toLowerCase());
    return matchesUni && matchesSearch;
  });

  // Filter semesters based on selected branch and search
  const filteredSemesters = semesters.filter((sem) => {
    const matchesBranch =
      currentBranchId === 'all' ||
      (sem.branchId?.id || sem.branchId) === currentBranchId;
    const matchesSearch =
      `Semester ${sem.number}`
        .toLowerCase()
        .includes(semSearch.toLowerCase()) ||
      sem.title?.toLowerCase().includes(semSearch.toLowerCase());
    return matchesBranch && matchesSearch;
  });

  const activeFiltersCount = [
    searchParams.get('universityId') &&
      searchParams.get('universityId') !== 'all',
    searchParams.get('branchId') && searchParams.get('branchId') !== 'all',
    searchParams.get('semesterId') && searchParams.get('semesterId') !== 'all',
  ].filter(Boolean).length;

  const updateFilter = (name, value) => {
    const params = new URLSearchParams(searchParams.toString());
    if (value && value !== 'all') {
      params.set(name, value);
    } else {
      params.delete(name);
    }
    // Reset dependent filters
    if (name === 'universityId') {
      params.delete('branchId');
      params.delete('semesterId');
    } else if (name === 'branchId') {
      params.delete('semesterId');
    }
    params.set('page', '1');
    router.push(`${pathname}?${params.toString()}`);
  };

  const clearFilters = () => {
    const params = new URLSearchParams(searchParams.toString());
    params.delete('universityId');
    params.delete('branchId');
    params.delete('semesterId');
    params.set('page', '1');
    router.push(`${pathname}?${params.toString()}`);
  };

  const renderFilterSection = (
    label,
    items,
    currentValue,
    paramName,
    searchValue,
    setSearch,
    placeholder
  ) => (
    <>
      <DropdownMenuLabel className="text-muted-foreground px-2 py-1.5 text-[10px] font-bold tracking-wider uppercase">
        {label}
      </DropdownMenuLabel>
      <DropdownMenuItem
        onClick={() => updateFilter(paramName, 'all')}
        className="flex cursor-pointer items-center justify-between"
      >
        <span>All {label}</span>
        {currentValue === 'all' && <Check className="text-primary size-3.5" />}
      </DropdownMenuItem>
      {items.length > 8 && (
        <div className="px-2 py-1.5">
          <input
            type="text"
            placeholder={placeholder}
            value={searchValue}
            onChange={(e) => setSearch(e.target.value)}
            className="border-input bg-background ring-offset-background placeholder:text-muted-foreground focus-visible:ring-ring flex h-8 w-full rounded-md border px-2 text-xs focus-visible:ring-1 focus-visible:outline-none"
            onClick={(e) => e.stopPropagation()}
          />
        </div>
      )}
      <div className="max-h-[200px] overflow-y-auto">
        {items.map((item) => (
          <DropdownMenuItem
            key={item.id}
            onClick={() => updateFilter(paramName, item.id)}
            className="flex cursor-pointer items-center justify-between"
          >
            <span>
              {item.name || item.shortName || `Semester ${item.number}`}
            </span>
            {currentValue === item.id && (
              <Check className="text-primary size-3.5" />
            )}
          </DropdownMenuItem>
        ))}
        {items.length === 0 && (
          <div className="text-muted-foreground px-3 py-2 text-xs">
            No {label.toLowerCase()} found
          </div>
        )}
      </div>
    </>
  );

  return (
    <div className="relative">
      <DropdownAction
        label="Filter Offerings"
        tooltip="Filter List"
        trigger={
          <Button variant="ghost" size="icon" className="relative size-10">
            <Filter className="size-4" />
            {activeFiltersCount > 0 && (
              <span className="bg-primary text-primary-foreground absolute top-1.5 right-1.5 flex size-4 items-center justify-center rounded-full text-[10px] font-bold">
                {activeFiltersCount}
              </span>
            )}
          </Button>
        }
      >
        {renderFilterSection(
          'Universities',
          filteredUniversities,
          currentUniId,
          'universityId',
          uniSearch,
          setUniSearch,
          'Search institutions...'
        )}

        <DropdownMenuSeparator className="my-2 border-b" />

        {renderFilterSection(
          'Branches',
          filteredBranches,
          currentBranchId,
          'branchId',
          branchSearch,
          setBranchSearch,
          'Search branches...'
        )}

        <DropdownMenuSeparator className="my-2 border-b" />

        {renderFilterSection(
          'Semesters',
          filteredSemesters,
          currentSemesterId,
          'semesterId',
          semSearch,
          setSemSearch,
          'Search semesters...'
        )}

        {activeFiltersCount > 0 && (
          <>
            <DropdownMenuSeparator className="my-2 border-b" />
            <DropdownMenuItem
              onClick={clearFilters}
              className="text-destructive focus:text-destructive focus:bg-destructive/5 cursor-pointer font-medium"
            >
              <X className="mr-2 size-3.5" />
              Clear Filters
            </DropdownMenuItem>
          </>
        )}
      </DropdownAction>
    </div>
  );
}
