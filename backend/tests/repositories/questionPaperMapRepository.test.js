...
describe('QuestionPaperMapRepository', () => {
  ...
  describe('create', () => {
    ...
    it('should throw ConflictError on duplicate paperId and questionId', async () => {
      ...
      await expect(
        questionPaperMapRepository.create(mappingData)
  ...
  describe('findByPaper', () => {
    it('should return paginated mappings for a paper', async () => {
      await questionPaperMapRepository.create({
        ...mappingData,
      ...
      await questionPaperMapRepository.create(mappingData);
      const result = await questionPaperMapRepository.findByPaper(paperId, {
        page: 1,
        limit: 10,
      });
  ...
  describe('findByQuestion', () => {
    it('should return mappings for a question', async () => {
      await questionPaperMapRepository.create(mappingData);
      const result = await questionPaperMapRepository.findByQuestion(
        questionId,
        { page: 1, limit: 10 }
      );
  ...
  describe('findByPaperAndQuestion', () => {
    it('should find specific mapping', async () => {
      await questionPaperMapRepository.create(mappingData);
      const mapping = await questionPaperMapRepository.findByPaperAndQuestion(
        paperId,
        questionId
      );
      expect(mapping.section).toBe('A');
  ...
  describe('update', () => {
    it('should update successfully', async () => {
      ...
      const updated = await questionPaperMapRepository.update(created.id, {
        marks: 10,
      });
  ...
  describe('delete', () => {
    it('should delete successfully', async () => {
      ...
      await expect(
        questionPaperMapRepository.findById(created.id)
